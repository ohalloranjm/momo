...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (newFatigue > 5) {
      ...
      if (original >= 5) {
        msg += ` is already at maximum fatigue, so they’re forced to mark ${mark} condition${
          mark > 1 ? 's' : ''
        }.`;
      } else {
        msg += ` marks ${
          mark - spillover
        }-fatigue, bringing them to 5-fatigue, and is forced to mark ${spillover} condition${
          spillover > 1 ? 's' : ''
        }.`;
      }
      ...
      if (conditionsMarked + spillover > 5) {
        ...
        await pc.save();
        msg +=
          ' They’re taken out! Run ``/takenout`` at the end of this combat exchange, or right now if you’re not in an exchange.';
    ...
    await pc.save();
    await interaction.followUp(
      `${pc.name} marks ${mark}-fatigue, bringing them to ${newFatigue}-fatigue.`
    );
  },
};
