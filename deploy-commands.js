...
const { alias } = require('./commands/set/condition');
...
for (const folder of commandFolders) {
    ...
    for (const file of commandFiles) {
        ...
        if ('data' in command && 'execute' in command) {
            ...
            if ('alias' in command) {
                const aliasedCommand = Object.assign(new SlashCommandBuilder(), command.data);
                aliasedCommand.setName(command.alias);
                commands.push(aliasedCommand.toJSON());
            }
        } else {
